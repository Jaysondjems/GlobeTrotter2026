...
function validateSearchQuery(query) {
  const errors = [];
  if (query.minBudget !== undefined && Number.isNaN(Number(query.minBudget))) {
    errors.push('minBudget must be numeric');
  }
  if (query.maxBudget !== undefined && Number.isNaN(Number(query.maxBudget))) {
    errors.push('maxBudget must be numeric');
  }
  if (
